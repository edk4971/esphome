"""ESPHome ``openai_responses`` external component.
...
openai_responses_ns = cg.esphome_ns.namespace("openai_responses")
OpenAIResponses = openai_responses_ns.class_(
    "OpenAIResponses", cg.Component
)
...
# OpenAIResponses component.
StartAction = openai_responses_ns.class_(
    "StartAction", automation.Action, cg.Parented.template(OpenAIResponses)
)
StopAction = openai_responses_ns.class_(
    "StopAction", automation.Action, cg.Parented.template(OpenAIResponses)
)
IsRunningCondition = openai_responses_ns.class_(
    "IsRunningCondition", automation.Condition, cg.Parented.template(OpenAIResponses)
)
...
CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(OpenAIResponses),
            ...
            # corrupts through cg.add() string escaping).
...
FINAL_VALIDATE_SCHEMA = cv.Schema(
    {
        cv.Optional(CONF_MICROPHONE): microphone.final_validate_microphone_source_schema(
            "openai_responses", sample_rate=16000
        ),
    },
...
async def to_code(config):
    ...
    # USE_OPENAI_RESPONSES_TOOLS is defined. We avoid passing the JSON
    ...
    if (tools_file := config.get(CONF_TOOLS_FILE)) is not None:
        ...
        header_path = CORE.relative_build_path("src", "openai_responses_tools.h")
        header_content = (
            "#pragma once\n"
        ...
        cg.add_define("USE_OPENAI_RESPONSES_TOOLS")
    ...
    if (mcp_servers := config.get(CONF_MCP_SERVERS)) is not None:
        ...
        cg.add_define("USE_OPENAI_RESPONSES_MCP")
...
openai_responses_ACTION_SCHEMA = cv.Schema(
    {cv.GenerateID(): cv.use_id(OpenAIResponses)}
)
...
@register_action(
    ...
    StartAction,
    openai_responses_ACTION_SCHEMA.extend(
        {
...
async def openai_responses_start_to_code(config, action_id, template_arg, args):
...
@register_action(
    ...
    StopAction,
    openai_responses_ACTION_SCHEMA,
    synchronous=True,
)
async def openai_responses_stop_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
    return var
...
@register_condition(
    "openai_responses.is_running",
    IsRunningCondition,
    openai_responses_ACTION_SCHEMA,
)
async def openai_responses_is_running_to_code(
...
_generic_start_schema = openai_responses_ACTION_SCHEMA.extend(
    {
        cv.Optional(CONF_SILENCE_DETECTION, default=True): cv.boolean,
        cv.Optional(CONF_WAKE_WORD): cv.templatable(cv.string),
    }
